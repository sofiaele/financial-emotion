...
def split_wav(input_wav, output_folder, segments_df):
    ...
    return segments_df
def make_utterances():
    csvs = [os.path.join('data/audio/', csv) for csv in os.listdir('data/audio/') if csv.endswith(".csv")]
    # for loop csvs
    for file in csvs:
        ...
        segments_df = split_wav(".".join(file.split(".")[:-1]) + ".wav", "data/audio/utterances", segments_df)
